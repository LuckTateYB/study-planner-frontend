import { Component, EventEmitter, Input, OnChanges, Output, SimpleChanges } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Exam } from '../../models/exam';
import { Course } from '../../models/course';

/**
 * Componente para registrar fechas de examen por curso.
 * Requiere que al menos un curso esté cargado.
 */
@Component({
  selector: 'app-exam-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="card">
      <div class="card-header">
        <span class="card-icon">📅</span>
        <h2 class="card-title">Fechas de Examen</h2>
        <span class="badge">{{ exams.length }}</span>
      </div>

      <!-- Aviso si no hay cursos cargados -->
      <p *ngIf="availableCourses.length === 0" class="empty-hint">
        ⚠ Primero agrega cursos en el paso anterior.
      </p>

      <ng-container *ngIf="availableCourses.length > 0">
        <div class="form-row">
          <div class="input-group">
            <label class="input-label">Curso</label>
            <select [(ngModel)]="selectedCourse" class="form-select">
              <option value="">-- Seleccionar curso --</option>
              <option *ngFor="let c of availableCourses" [value]="c.name">{{ c.name }}</option>
            </select>
          </div>

          <div class="input-group">
            <label class="input-label">Fecha del examen</label>
            <input
              type="date"
              [(ngModel)]="selectedDate"
              [min]="today"
              class="form-input"
            />
          </div>

          <button
            class="btn btn--primary"
            (click)="addExam()"
            [disabled]="!selectedCourse || !selectedDate"
          >
            + Agregar
          </button>
        </div>

        <p *ngIf="showError" class="form-error">⚠ Selecciona un curso y una fecha válida.</p>

        <!-- Lista de exámenes -->
        <ul class="item-list" *ngIf="exams.length > 0">
          <li *ngFor="let exam of exams; let i = index" class="item-row">
            <span class="exam-course">{{ exam.course }}</span>
            <span class="exam-date">📅 {{ formatDate(exam.date) }}</span>
            <button class="btn-remove" (click)="removeExam(i)" title="Eliminar">✕</button>
          </li>
        </ul>

        <p *ngIf="exams.length === 0" class="empty-hint">
          Agrega al menos un examen para continuar.
        </p>
      </ng-container>
    </div>
  `,
  styles: [`
    .card { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 1.5rem; }
    .card-header { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1.25rem; }
    .card-icon { font-size: 1.3rem; }
    .card-title { font-size: 1.1rem; font-weight: 700; color: var(--text-primary); margin: 0; flex: 1; }
    .badge { background: var(--accent); color: white; border-radius: 99px; padding: 2px 10px; font-size: 0.78rem; font-weight: 700; }
    .form-row { display: flex; gap: 0.75rem; flex-wrap: wrap; align-items: flex-end; }
    .input-group { flex: 1; min-width: 160px; }
    .input-label { display: block; font-size: 0.75rem; color: var(--text-secondary); margin-bottom: 4px; }
    .form-input, .form-select {
      width: 100%; padding: 0.55rem 0.75rem; border: 1.5px solid var(--border);
      border-radius: 8px; background: var(--bg); color: var(--text-primary);
      font-size: 0.9rem; transition: border-color 0.2s;
    }
    .form-input:focus, .form-select:focus { outline: none; border-color: var(--accent); }
    .btn--primary {
      padding: 0.55rem 1.2rem; background: var(--accent); color: white;
      border: none; border-radius: 8px; font-weight: 600; cursor: pointer;
      transition: opacity 0.2s; white-space: nowrap;
    }
    .btn--primary:disabled { opacity: 0.4; cursor: not-allowed; }
    .btn--primary:not(:disabled):hover { opacity: 0.85; }
    .form-error { color: var(--danger); font-size: 0.82rem; margin-top: 0.5rem; }
    .item-list { list-style: none; padding: 0; margin: 1rem 0 0; display: flex; flex-direction: column; gap: 0.5rem; }
    .item-row {
      display: flex; align-items: center; gap: 0.75rem;
      background: var(--bg); border: 1px solid var(--border);
      border-radius: 8px; padding: 0.5rem 0.75rem;
    }
    .exam-course { flex: 1; font-size: 0.9rem; color: var(--text-primary); font-weight: 600; }
    .exam-date { font-size: 0.85rem; color: var(--text-secondary); }
    .btn-remove {
      background: none; border: none; color: var(--text-secondary);
      cursor: pointer; font-size: 0.85rem; padding: 2px 6px;
      border-radius: 4px; transition: color 0.2s, background 0.2s;
    }
    .btn-remove:hover { color: var(--danger); background: rgba(239,68,68,0.1); }
    .empty-hint { color: var(--text-secondary); font-size: 0.85rem; font-style: italic; margin-top: 1rem; }
  `]
})
export class ExamFormComponent implements OnChanges {

  /** Cursos disponibles para seleccionar (recibidos desde el padre) */
  @Input() availableCourses: Course[] = [];

  /** Emite la lista de exámenes hacia el componente padre */
  @Output() examsChange = new EventEmitter<Exam[]>();

  exams: Exam[] = [];
  selectedCourse = '';
  selectedDate = '';
  showError = false;

  /** Fecha mínima: hoy */
  today = new Date().toISOString().split('T')[0];

  /** Actualiza la lista de exámenes si se eliminan cursos */
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['availableCourses']) {
      const names = this.availableCourses.map(c => c.name);
      this.exams = this.exams.filter(e => names.includes(e.course));
      this.examsChange.emit(this.exams);
    }
  }

  /** Agrega un examen si el formulario es válido */
  addExam(): void {
    if (!this.selectedCourse || !this.selectedDate) {
      this.showError = true;
      return;
    }
    this.showError = false;
    this.exams = [...this.exams, { course: this.selectedCourse, date: this.selectedDate }];
    this.selectedCourse = '';
    this.selectedDate = '';
    this.examsChange.emit(this.exams);
  }

  /** Elimina un examen por índice */
  removeExam(index: number): void {
    this.exams = this.exams.filter((_, i) => i !== index);
    this.examsChange.emit(this.exams);
  }

  /** Formatea una fecha ISO a DD/MM/YYYY */
  formatDate(iso: string): string {
    const [y, m, d] = iso.split('-');
    return `${d}/${m}/${y}`;
  }
}
